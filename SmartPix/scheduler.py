...
class SmartPixCache(DynamicCache):
    def __init__(self, 
...
class SmartPixScheduler:
    ...
    def __call__(self, z, func, model_kwargs, use_kv_cache: bool=True, offload_kv_cache: bool=True):
        ...
        if isinstance(model_kwargs['input_ids'], list):
            cache = [SmartPixCache(num_tokens_for_img, offload_kv_cache) for _ in range(len(model_kwargs['input_ids']))] if use_kv_cache else None
        else:
            cache = SmartPixCache(num_tokens_for_img, offload_kv_cache) if use_kv_cache else None
        results = {}
