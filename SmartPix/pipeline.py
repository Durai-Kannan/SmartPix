...
from SmartPix import SmartPix, SmartPixProcessor, SmartPixScheduler
...
class SmartPixPipeline:
    def __init__(
        self,
        vae: AutoencoderKL,
        model: SmartPix,
        processor: SmartPixProcessor,
    ...
    def from_pretrained(cls, model_name, vae_path: str=None):
        ...
        model = SmartPix.from_pretrained(model_name)
        processor = SmartPixProcessor.from_pretrained(model_name)
    ...
    def __call__(
        ...
        if max_input_image_size != self.processor.max_image_size:
            self.processor = SmartPixProcessor(self.processor.text_tokenizer, max_image_size=max_input_image_size)
        self.model.to(dtype)
        if offload_model:
        ...
        scheduler = SmartPixScheduler(num_steps=num_inference_steps)
